...
describe('GET /api/cron/reminders', () => {
  ...
  it('finds upcoming appointments and creates reminders', async () => {
    ...
    // scheduledFor should be 1 hour before the appointment
  ...
  it('skips appointments that already have pending/sent reminders', async () => {
    ...
    expect(prisma.appointmentReminder.create).toHaveBeenCalledOnce()
...
describe('GET /api/cron/collections', () => {
  ...
  it('does not create notification if no user with matching role exists', async () => {
    ...
    const body = await res.json()
    expect(body.summary[0].tier1).toBe(1)
...
describe('GET /api/cron/inventory', () => {
  ...
  it('creates WARNING alerts for items at reorder level but above minimum', async () => {
    ...
    const createCalls = vi.mocked(prisma.aIInsight.create).mock.calls
    // Only 1 alert: WARNING for approaching reorder (not critical because currentStock > minimumStock)
    ...
    expect(createCalls[0][0].data.description).toContain('Composite')
    expect(createCalls[0][0].data.description).not.toContain('Cement')
  })
  ...
  it('creates alerts for expiring batches', async () => {
    ...
    const createCalls = vi.mocked(prisma.aIInsight.create).mock.calls
    // Only the expiring batch alert (no stock alerts since everything is above reorder)
    expect(createCalls).toHaveLength(1)
    expect(createCalls[0][0].data).toMatchObject({
      hospitalId: 'h1',
      category: 'INVENTORY',
    ...
    expect(createCalls[0][0].data.description).toContain('Cement')
    expect(createCalls[0][0].data.description).toContain('FEFO')
  })

  it('creates all 3 alert types when applicable', async () => {
    vi.mocked(prisma.hospital.findMany).mockResolvedValue([{ id: 'h1' }] as any)
    ...
    const body = await res.json()
    expect(body.results[0].alertsCreated).toBe(3)
    ...
    expect(severities.filter((s) => s === 'WARNING')).toHaveLength(2)
  })
