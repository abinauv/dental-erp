...
describe('ChatWidget', () => {
  ...
  it('clicking a hint sets the input value', async () => {
    ...
    await act(async () => {
      fireEvent.click(screen.getByText('Patient summary'))
    })

    const textarea = screen.getByPlaceholderText('Type a message\u2026') as HTMLTextAreaElement
    expect(textarea.value).toBe('Patient summary')
  })
})
  ...
  it('calls /api/ai/clinical with correct parameters', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/ai/clinical', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ type: 'patient_summary', patientId: 'patient-123' }),
      }))
    })
  })
})
...
describe('ReportBuilder', () => {
  ...
  it('submits query and shows results table', async () => {
    ...
    mockFetchResponse({
      ...
      message: 'Here is your revenue breakdown.',
    })
  ...
  it('shows Export JSON button when results are present', async () => {
    ...
    mockFetchResponse({
      success: true,
      results: [{ procedure: 'Root Canal', revenue: 50000 }],
    })
